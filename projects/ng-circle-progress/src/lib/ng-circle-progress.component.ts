...
@Component({
  ...
  template: `
    ...
    >
      ...
      <text
        *ngIf="
          !options.showImage && (options.showTitle || options.showUnits || options.showSubtitle)
        "
...
export class CircleProgressComponent implements OnChanges, OnInit, OnDestroy {
  ...
  draw = (percent: number) => {
    ...
    if (this.options.showBackground) {
      boxSize +=
        this.options.backgroundStrokeWidth * 2 + this.max(0, this.options.backgroundPadding * 2);
    }
